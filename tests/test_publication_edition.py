...
def repository(tmp_path: Path) -> Path:
    ...
    write(tmp_path, publication.PREVIOUS + "/MANUSCRIPT.md", "# Historical source\n\nOriginal.\n")
    write(tmp_path, "research/registry/questions.yaml", "- id: RQ-FIXTURE-001\n  status: open\n  hypotheses: [H-FIXTURE-001-A]\n  question: Test?\n")
    write(tmp_path, "research/registry/hypotheses.yaml", "- id: H-FIXTURE-001-A\n  status: untested\n  question: RQ-FIXTURE-001\n")
    write(tmp_path, "research/publications/catalog.json", '{"publications": []}')
    write(tmp_path, "docs/unclassified.md", "# Retained idea\n\nNot discarded.\n")
    subprocess.run(["git", "-C", str(tmp_path), "add", "."], check=True)
    subprocess.run(["git", "-C", str(tmp_path), "commit", "-qm", "fixture baseline"], check=True)
    ...
    write(tmp_path, "scripts/build_publication_edition.py", (ROOT / "scripts/build_publication_edition.py").read_text(encoding="utf-8"))
    cfg = json.loads((tmp_path / publication.EDITION / "edition.json").read_text(encoding="utf-8"))
    cfg["baseline_commit"] = baseline
    (tmp_path / publication.EDITION / "edition.json").write_text(publication.json_text(cfg), encoding="utf-8")
    return tmp_path
...
def test_registry_projection_preserves_source_object(repository: Path) -> None:
    ...
    original = yaml.safe_load((repository / "research/registry/questions.yaml").read_text(encoding="utf-8"))[0]
...
def test_unknown_citation_invalid_path_and_chat_promotion_fail(repository: Path) -> None:
