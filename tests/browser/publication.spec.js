...
for (const port of [4174, 4175]) {
 for (const popup of [true, false]) {
  test(`publication ${port} ${popup ? "popup" : "inline"}: current v1.8 and frozen v1.5 remain separately reachable`, async ({ page }) => {
    ...
    await expect(portalLinks.locator('[data-portal="hf-source"]')).toHaveAttribute('href', 'https://huggingface.co/superdigger/MHRN');
    await expect(portalLinks.locator('[data-portal="hf-space"]')).toHaveAttribute('href', 'https://huggingface.co/spaces/superdigger/MHRN-Space');
    await expect(portalLinks.locator('[data-portal="hf-data"]')).toHaveAttribute('href', 'https://huggingface.co/datasets/superdigger/MHRN-Research-Data');
