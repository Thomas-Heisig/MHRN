...
def main() -> int:
    ...
    protected = {
        ...
        for folder in ("research/experiments", "research/publications", "research/preregistrations", "research/registry/evidence")
    ...
    def run(name: str, command: list[str], timeout: int = 1800) -> int:
        ...
        with log.open("w", encoding="utf-8") as stream:
            try:
                result = subprocess.run(command, cwd=ROOT, stdout=stream, stderr=subprocess.STDOUT, timeout=timeout, check=False)
    ...
    run("campaign", [sys.executable, "scripts/empirical_campaign.py", "--output", str(campaign), "--campaign-id", CAMPAIGN], 2400)
    run("stage3", [sys.executable, "scripts/run_stage3_reference.py", "--output", str(output / "stage3.json")])
    ...
    changed = [str(path.relative_to(ROOT)) for path, digest in protected.items() if not path.is_file() or hashlib.sha256(path.read_bytes()).hexdigest() != digest]
    if changed:
        raise RuntimeError("Historical scientific artifacts changed: " + repr(changed))
    completion = campaign / "completion.json"
    if not completion.exists() or not json.loads(completion.read_text())["source_unchanged"]:
        raise RuntimeError("Campaign did not finish with unchanged source; no publication")
    ...
    shutil.copy2(output / "stage3.json", ROOT / "research/generated/verification/plastic_neural_tissue_reference_alpha3.json")
    if os.environ.get("GITHUB_REF_NAME") == BRANCH:
        subprocess.run(["git", "config", "user.name", "MHRN audit (AI-assisted)"], cwd=ROOT, check=True)
        subprocess.run(["git", "config", "user.email", "41898282+github-actions[bot]@users.noreply.github.com"], cwd=ROOT, check=True)
        subprocess.run(["git", "add", "research/experiments/" + CAMPAIGN, "research/generated", "docs/05-quality/scientific-audit-20260913.json", "tests/test_baseline.json"], cwd=ROOT, check=True)
        subprocess.run(["git", "commit", "-m", "research: retain complete alpha3 audit, negative outcomes and source-bound checks"], cwd=ROOT, check=True)
        subprocess.run(["git", "push", "origin", "HEAD:refs/heads/" + BRANCH], cwd=ROOT, check=True)
    subprocess.run(["git", "bundle", "create", str(output / "audit-source.bundle"), "--all"], cwd=ROOT, check=True)
