#!/usr/bin/env python3
"""Execute EXP-S6-SEM-CL-002 only after explicit hash-bound authorization."""
...
def _require_authorization(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise RuntimeError(
            "CL-002 execution authorization is absent; empirical run is blocked"
        )
    ...
    if authorization.get("execution_authorized") is not True:
        raise RuntimeError("CL-002 execution authorization is false")
    return authorization


def _verify_freeze(path: Path) -> dict[str, str]:
    if not path.exists():
        raise RuntimeError("CL-002 freeze manifest is absent")
