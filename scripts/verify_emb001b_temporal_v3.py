#!/usr/bin/env python3
"""Verify persisted DATA for EXP-EMB001B-TEMPORAL-V3-20260920."""
...
def main() -> int:
    ...
    assert manifest["research_question"] == "RQ-EMB-001"
    assert manifest["hypothesis"] == "H-EMB-001-B"
    assert manifest["protocol"] == "embodied_closed_loop_temporal_shift_v3"
    ...
    for seed in seeds:
        ...
        shift100 = rows[(seed, "yoked_replay_shift100")]
        assert exact["donor_multiset_sha256"] == shift20["donor_multiset_sha256"]
        assert exact["donor_multiset_sha256"] == shift100["donor_multiset_sha256"]
        assert exact["donor_tape_sha256"] != shift20["donor_tape_sha256"]
        assert exact["donor_tape_sha256"] != shift100["donor_tape_sha256"]
        assert shift20["donor_shift_ticks"] == 20
        assert shift100["donor_shift_ticks"] == 100
