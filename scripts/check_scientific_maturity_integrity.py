...
def main() -> int:
    ...
    if isinstance(weights, dict):
        ...
        require(abs(sum(float(value) for value in weights.values()) - 1.0) < 1e-9, "scientific-progress weights must sum to 1")
    if isinstance(stages, list):
        require(
            [stage.get("stage") for stage in stages if isinstance(stage, dict)] == list(range(11)),
        ...
        for stage in stages:
            ...
            require(isinstance(stage.get("criteria"), list) and bool(stage.get("criteria")), f"stage {stage.get('stage')} missing scientific criteria")
        stage6 = next((stage for stage in stages if isinstance(stage, dict) and stage.get("stage") == 6), None)
        ...
        if isinstance(stage6, dict):
            ...
            require("kein hierarchisches predictive coding" in boundary, "stage 6 must deny completed hierarchical predictive coding")
    ...
    if len(current) == 1:
        ...
        require(item.get("edition_status") == "current_wip", "edition 1.7 must be visibly marked current_wip")
        require(item.get("entrypoint", "").endswith("v1.7/MANUSCRIPT.md"), "Publication Viewer must open the current v1.7 manuscript")
        require(item.get("automatic_evidence_promotion") is False, "current publication must not auto-promote evidence")
        require(item.get("predecessor") == "PUB-RECURSIVE-EPISTEMICS-20260915-V1.6", "edition 1.7 predecessor mismatch")
        require(item.get("inherits_empirical_edition") == "PUB-RECURSIVE-EPISTEMICS-20260913-V1.5", "edition 1.7 must retain edition 1.5 empirical provenance")
    ...
    require(catalog.get("frozen_empirical_baseline") == "PUB-RECURSIVE-EPISTEMICS-20260913-V1.5", "frozen 1.5 baseline pointer missing")

    v17 = load_json(f"{V17}/manifest.json")
    require(v17.get("historical_data_modified") is False, "v1.7 may not rewrite historical data")
    require(v17.get("accepted_evidence") is False, "v1.7 may not manufacture accepted evidence")
    require(v17.get("automatic_evidence_promotion") is False, "v1.7 may not auto-promote evidence")
    require(v17.get("inherited_campaign") == "EXP-EMP-20260913-A3", "v1.7 campaign provenance mismatch")
    require(v17.get("predecessor_edition", "").endswith("v1.6"), "v1.7 predecessor edition missing")
    require(v17.get("viewer_entrypoint") == "MANUSCRIPT.md", "v1.7 manifest viewer entrypoint mismatch")

    v16 = load_json(f"{V16}/manifest.json")
    require(v16.get("historical_data_modified") is False, "v1.6 provenance must remain intact")
    require(v16.get("accepted_evidence") is False, "v1.6 may not manufacture accepted evidence")
    require((ROOT / V15 / "manifest.json").is_file(), "frozen 1.5 manifest is missing")
    require((ROOT / "research/publications/FROZEN_V1.5.md").is_file(), "stable frozen 1.5 entrypoint is missing")
    require((ROOT / "research/publications/CURRENT.md").is_file(), "stable current publication pointer is missing")
    ...
    require(isinstance(publication, dict) and publication.get("edition") == "1.7", "project identity publication edition mismatch")
    require(isinstance(publication, dict) and publication.get("edition_status") == "current_wip", "project identity must expose WIP status")
    ...
    require(isinstance(scope, dict) and scope.get("candidate_contribution_novelty") == "requires_prior_art_review", "novelty uncertainty boundary missing")
    require(isinstance(governance, dict) and governance.get("audit") == "scripts/audit_document_governance.py", "document governance audit is not registered")
    ...
    author_position = (ROOT / V17 / "AUTHOR_POSITION.md").read_text(encoding="utf-8").lower()
    ...
    require("frozen 1.5" in manuscript, "current manuscript must identify frozen 1.5")
    require("neuheit" in manuscript and "prior-art" in manuscript, "current manuscript must expose novelty uncertainty")
    require("freier wissenstransfer" in author_position and "attribution" in author_position, "author position must preserve the knowledge-transfer/attribution distinction")
    ...
    print("Internal consistency verified; no claim of plagiarism-freedom, novelty, or scientific acceptance is made.")
    return 0
