...
def main() -> int:
    ...
    for question_id in (
        ...
        if question_id not in rows:
            errors.append(f"missing meta question {question_id}")
        elif rows[question_id]["direction"] != "DIR-11":
            errors.append(f"{question_id} is not mapped to DIR-11")

    studies_dir = ROOT / "research/meta/studies"
    for path in sorted(studies_dir.glob("*.json")):
        data = json.loads(path.read_text(encoding="utf-8"))
        if (
            data.get("execution_authorized") is not False
            or data.get("scientific_evidence") is not False
        ):
            errors.append(
                f"{path}: design artifact illegally authorizes execution/evidence"
            )
