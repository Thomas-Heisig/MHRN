#!/usr/bin/env python3
"""Run the preregistered Stage-1 topology v3 R1 time-resolved replication."""
...
EXP_ID = "EXP-S1-TOPO-V3-R1-20260918"
...
def analyze(evaluation: list[dict[str, Any]], prereg: dict[str, Any]) -> dict[str, Any]:
    ...
    for endpoint in PRIMARY_ENDPOINTS:
        for left, right in PRIMARY_CONTRASTS:
            ...
            boot_seed = int(
                hashlib.sha256(
                    f"{EXP_ID}:{endpoint}:{left}:{right}".encode()
                ).hexdigest()[:12],
                16,
            )
            primary_tests.append(
                {
                    ...
                    "n_pairs": len(differences),
                    "nonzero_pairs": sum(
                        abs(value) > 1e-12 for value in differences
                    ),
                    "median_difference": float(statistics.median(differences)),
                    "mean_difference": float(statistics.fmean(differences)),
    ...
    return {
        ...
        "primary_multiple_testing": "single Holm family across all 10 time-resolved tests",
...
def render_report(
    ...
    lines = [
        f"# {EXP_ID}: topology_propagation_v3_r1_time_resolved_replication",
        "",
        "Stage 1 - Kleines SNN",
