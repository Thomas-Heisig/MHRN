...
def bootstrap_ci(values: list[float]) -> list[float]:
    ...
    medians = [statistics.median([values[rng.randrange(len(values))] for _ in values]) for _ in range(5000)]
...
def sign_test_p(values: list[float]) -> float:
    ...
    return min(1.0, 2.0 * sum(math.comb(len(nonzero), k) for k in range(tail + 1)) / (2 ** len(nonzero)))
...
def network(config: dict[str, Any], seed: int, weight: float) -> tuple[NeuralNetwork, dict[str, int]]:
    values = dict(config)
    ...
    ids = {name: net.add_neuron((i, 0, 0, 0, 0)) for i, name in enumerate(
    ...
    for source, target in (
        ("input_A", "hidden_A"), ("hidden_A", "output_A"),
        ("input_B", "hidden_B"), ("hidden_B", "output_B"),
    ):
...
def simulate(config: dict[str, Any], prereg: dict[str, Any], seed: int, arm: str, order: str) -> dict[str, Any]:
    ...
    return {
        "seed": seed, "arm": arm, "order": order,
        "node_count": net.neuron_count, "edge_count": net.synapse_count,
        "synaptic_weight": weight, "stimulus_current": current,
        "logical_schedule": logical, "actual_schedule": actual,
        "total_injected_charge": 2.0 * current,
        "first_output_tick": first, "decoded_order": decoded,
        "task_success": decoded == order, "output_sequence": output_sequence,
        "total_spikes": spikes, "delivered_events": events,
        "state_digest_before": before, "state_digest_after": canonical_state_digest(net),
    }
...
def analyze(runs: list[dict[str, Any]], prereg: dict[str, Any]) -> dict[str, Any]:
    ...
    rows = []
    for seed in seeds:
        intact = statistics.fmean(float(lookup[(seed, "intact", o)]["task_success"]) for o in ORDER_TRIALS)
        destroyed = statistics.fmean(float(lookup[(seed, "identity_destroyed", o)]["task_success"]) for o in ORDER_TRIALS)
        rows.append({
            "seed": seed, "intact_order_accuracy": intact,
            "identity_destroyed_order_accuracy": destroyed,
            "paired_accuracy_delta": intact - destroyed,
            "intact_simultaneous_success": bool(lookup[(seed, "intact", "simultaneous")]["task_success"]),
        })
    ...
    summary = {
        ...
        "intact_simultaneous_success_fraction": statistics.fmean(float(r["intact_simultaneous_success"]) for r in rows),
    ...
    gates = {
        "intact_order_accuracy": summary["intact_order_accuracy_median"] >= float(rule["minimum_intact_order_accuracy"]),
        "identity_destroyed_ceiling": summary["identity_destroyed_order_accuracy_median"] <= float(rule["maximum_identity_destroyed_order_accuracy"]),
        "simultaneous_control": summary["intact_simultaneous_success_fraction"] >= float(rule["minimum_intact_simultaneous_success_fraction"]),
        "paired_delta": summary["paired_accuracy_delta_median"] >= float(rule["minimum_paired_accuracy_delta"]),
        "paired_sign_test": summary["paired_sign_test_p"] < float(prereg["analysis"]["alpha"]),
        "bootstrap_ci_excludes_zero": summary["paired_accuracy_delta_bootstrap_ci95"][0] > 0.0,
    }
    return {
        "per_seed": rows, "summary": summary, "gates": gates,
        "status": "SUPPORTED_WITHIN_PREREGISTERED_PROTOCOL" if all(gates.values()) else "NO_PREDEFINED_SUPPORT_DETECTED",
    }


def main() -> int:
    prereg = read_json(PREREG)
    if prereg["protocol"] != "stage1_two_channel_temporal_order_v2" or prereg["execution_authorized"] is not True:
    ...
    runs = [
        ...
        for seed in prereg["evaluation"]["seeds"] for arm in ARMS for order in ORDERS
    ]
    seeds = list(map(int, prereg["evaluation"]["seeds"]))
    integrity_checks = {
        "run_count": len(runs) == len(seeds) * 6,
        "coverage": all(sum(r["seed"] == s and r["arm"] == a and r["order"] == o for r in runs) == 1 for s in seeds for a in ARMS for o in ORDERS),
        "network_budget": all(r["node_count"] == 6 and r["edge_count"] == 4 for r in runs),
        "paired_parameters": all(len({(round(r["synaptic_weight"], 12), round(r["stimulus_current"], 12), round(r["total_injected_charge"], 12)) for r in runs if r["seed"] == s}) == 1 for s in seeds),
        "fresh_seeds": set(seeds).isdisjoint(range(101, 121)) and set(seeds).isdisjoint(range(2101, 2121)) and set(seeds).isdisjoint(range(6201, 6221)),
    }
    ...
    write_json(stats, {"schema_version": 1, "experiment_id": EXP_ID, "analysis": analysis, "integrity": integrity, "claim_boundary": prereg["claim_boundary"]})
    manifest = {
        "schema_version": 1, "experiment_id": EXP_ID, "stage": 1,
        "research_question": "RQ-TEMP-002", "hypothesis": "H-TEMP-002-A",
        "protocol": prereg["protocol"], "experiment_status": "completed" if integrity["pass"] else "not_tested",
        "result_status": analysis["status"], "scientific_evidence": False,
        "automatic_evidence_promotion": False, "human_review_status": "PENDING",
        "independent_replication": False, "source_freeze": source,
        "environment": {"python": sys.version, "platform": platform.platform(), "os_name": os.name},
        "design": {"seed_count": len(seeds), "run_count": len(runs), "arms": list(ARMS), "orders": list(ORDERS), "node_count": 6, "edge_count": 4},
        "results": analysis["summary"], "integrity": integrity,
        "artifacts_sha256": {
            "preregistration": sha256(PREREG), "historical_v1_preregistration": sha256(HISTORICAL),
            "config": sha256(CONFIG), "runner_source": sha256(Path(__file__).resolve()),
            "evaluation_data": sha256(data), "statistics": sha256(stats),
        },
        "claim_boundary": prereg["claim_boundary"],
    }
    write_json(OUT / "manifest.json", manifest)
    write_json(OUT / "review_request.json", {
        "schema_version": 1, "experiment_id": EXP_ID, "reviewer_type_required": "human",
        "human_review_status": "PENDING", "scientific_evidence": False,
        "automatic_evidence_promotion": False, "independent_replication": False,
...
"""
    ...
    checks = {
        "analysis/statistics.json": sha256(stats), "data/evaluation.json": sha256(data),
        "manifest.json": sha256(OUT / "manifest.json"), "report.md": sha256(OUT / "report.md"),
    ...
    print(json.dumps({"experiment_id": EXP_ID, "status": analysis["status"], "integrity": integrity["pass"], "runs": len(runs), **s}, sort_keys=True))
    return 0
