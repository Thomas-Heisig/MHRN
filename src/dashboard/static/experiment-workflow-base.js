...
export class ExperimentWorkflowPanel {
  ...
  async _loadExperimentCollections() {
    ...
    try {
      ...
      const activeSeriesItems = seriesItems.filter((item) => !item.archived);
      const archivedSeriesItems = seriesItems.filter((item) => item.archived);
      const archivedExperimentItems = archivedItems.filter((item) => item.archive_type !== "series");
      ...
      if (seriesCount) seriesCount.textContent = `${activeSeriesItems.length + archivedSeriesItems.length} Reihen`;
      if (activeCount) activeCount.textContent = `${activeItems.length} aktiv`;
      if (archivedCount) archivedCount.textContent = `${archivedExperimentItems.length + archivedSeriesItems.length} archiviert`;
      const archiveSummaryCount = byId("workflow-archive-summary-count");
      if (archiveSummaryCount) archiveSummaryCount.textContent = `${archivedExperimentItems.length + archivedSeriesItems.length} archiviert`;
      const series = byId("workflow-experiment-series");
      if (series) series.innerHTML = activeSeriesItems.length ? activeSeriesItems.map((item) => this._experimentSeriesItem(item)).join("") : '<p class="experiment-library-empty">Keine aktive Experimentreihe.</p>';
      const archivedSeries = byId("workflow-archived-series");
      if (archivedSeries) archivedSeries.innerHTML = archivedSeriesItems.length ? archivedSeriesItems.map((item) => this._experimentSeriesItem(item)).join("") : '<p class="experiment-library-empty">Keine archivierten Reihen.</p>';
  ...
  _experimentLibraryItem(item, archived) {
    ...
    const rawPath = artifacts.data_index || artifacts.raw_run_index || artifacts.raw_data;
    if (rawPath) resultButtons.push(`<button type="button" class="btn-small exp-open-btn" data-experiment-open="raw" data-experiment-id="${expId}" data-artifact-path="${escapeHtml(rawPath)}" title="Rohdaten">🗂 Rohdaten</button>`);
  ...
  _useExperimentDetails(item) {
    ...
    const setValue = (control, value) => {
      if (control && value != null && value !== "") control.value = Array.isArray(value) ? value.join(",") : String(value);
    };
    setValue(this.elements.experimentId, item.experiment_id || item.id);
    setValue(this.elements.question, questions[0] || manifest.research_question);
    this._renderHypotheses();
    setValue(this.elements.hypothesis, hypotheses[0] || manifest.hypothesis);
    setValue(this.elements.title, manifest.title || manifest.experiment_title || item.experiment_id || item.id);
    setValue(this.elements.conditions, manifest.conditions || manifest.experimental_design?.conditions);
    setValue(this.elements.seeds, manifest.seeds);
    setValue(this.elements.ticks, manifest.ticks || manifest.requested_ticks);
    setValue(this.elements.notes, manifest.notes || manifest.claim_boundary || "Übernommen aus bestehendem Experiment; vor Ausführung prüfen.");
    setValue(this.elements.protocol, manifest.protocol || manifest.protocol_id);
    this._alignProtocolWithQuestion();
    this._applyProtocol();
    this._renderContract();
