...
**STAGE1-TEMPORAL-ORDER-LINE-002** is separate from the topology baseline: `RQ-TEMP-002 / H-TEMP-002-A`, experiment `EXP-S1-TEMP-ORDER-V2-20260919`. The design uses a six-neuron acyclic SNN, 20 seeds and 120 runs with intact versus identity-destroyed arms plus simultaneous control. Its status is `SUPPORTED_WITHIN_PREREGISTERED_PROTOCOL`; Human Review is pending, scientific EVID is false, and independent replication is false.

This line is scientifically useful because it contains an information-destroying control and a fixed decoding task, but it is not an independent replication of the topology line and does not establish learning, memory, cognition, scaling or general temporal reasoning.

## Remaining Stage-1 work

1. complete Human Review of Temporal-Order V2;
2. define a scoped claim and prospective EvidenceEngine-compatible promotion path for the topology line if EVID registration is desired;
3. obtain independently authored / independently controlled replication;
